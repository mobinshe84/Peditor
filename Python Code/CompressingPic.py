    ...
    def WriteInImage(self, array, w, h, d):
        ...
        for i in range(h):
            ...
            for color in range(d):
                ...
                file.write("* ")
        x += 1
        ReloadCompressing(100)
        file.close()
        
        print('----- Done -----')
        decodeThread = threading.Thread(target=DecodeImg.__init__)
        decodeThread.start()
...
class Compressing(QWidget):
    def setupUi(self, Peditor):
        ...
        self.DecodeButton.clicked.connect(self.DecodeClicked)
    ...
    def DecodeClicked(self):
        ...
        msg = QMessageBox()
        msg.setWindowTitle("Peditor")
        msg.setText("Compress don't completed! wait...")
