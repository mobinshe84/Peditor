...
class Ui_CompressPic(QWidget):
    ...
    def CompressClicked(self):
        try:
            ...
            x = msg.exec_()
            return
        SavePicMessage = QMessageBox()
        SavePicMessage.setWindowTitle("Peditor")
        SavePicMessage.setText("You have to defining us the location of the output!")
        SavePicMessage.setIcon(QMessageBox.Warning)
        x = SavePicMessage.exec_()
        self.SavePic()
        ...
        CompressThread = threading.Thread(target = CompressingPic.EncodeImg.LoadImage, args = [CompressingPic.EncodeImg, Im, SaveAddress])
